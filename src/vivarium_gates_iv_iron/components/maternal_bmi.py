...
class MaternalBMIExposure:
    ...
    def on_initialize_simulants(self, pop_data: SimulantData):
        ...
        pregnancy_status = (self.population_view
                            .subview(['pregnancy_status'])
                            .get(pop_data.index)
                            .pregnancy_status)
    ...
    def on_time_step_cleanup(self, event: Event):
        ...
        newly_pregnant = pop.loc[pop['pregnancy_status'] == models.PREGNANT_STATE].index
        bmi.loc[newly_pregnant] = self.sample_bmi(
            pop.loc[newly_pregnant, 'maternal_bmi_propensity']
        )

        newly_not_pregnant = pop[pop['pregnancy_status'] == models.NOT_PREGNANT_STATE].index
